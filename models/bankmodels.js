...
//this function searches through transfers and returns a sum of the 'amount' column separately for all instances of 'Debit' and 'Credit'
function getBankTransferTotalsByDate(id,bankName,searchDate) {

    let queryParamater = `SELECT SUM(CASE WHEN user_id=${id} THEN (CASE WHEN Transaction_timestamp < ${searchDate} THEN (CASE WHEN Credit='${bankName}' THEN -amount END) END) ELSE 0 END) AS TransferOut, SUM((CASE WHEN user_id = ${id} THEN (CASE WHEN Transaction_timestamp < ${searchDate} THEN (CASE WHEN Debit='${bankName}' THEN amount END) END) ELSE 0 END)) AS TransferIn FROM transfers`
...
//this function searchs through transfers and returns the 5 most recent transactions (by transaction date, not record date).
function searchTopFiveTransfers (id, bankName,searchDate) {
    return new Promise((resolve, reject) => {
        knex('transfers')
        .where(function(){
            this.where('user_id',id)
